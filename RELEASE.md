...
#### c. Regenerate `package-lock.json`

```bash
npm install
```

This updates the lockfile to match the new version in `package.json`.
...
| `package-lock.json` | `version` and `packages[""].version` bumped (regenerated by `npm install`) |
...
- [ ] `CHANGELOG.md` updated (Unreleased → versioned, fresh Unreleased block at top)
