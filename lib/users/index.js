...
exports.create = function (options) {
  ...
  var listAllUsers = (getOptions, callback) => {
    var requestOptions = _.extend({}, optionsToSend, getOptions);
  ...
  var upgradeUser = (postOptions, callback) => {
    var urlOptions = { url: buildUserUpgradeUrl(postOptions) };
    return requestor.post(_.extend({}, optionsToSend, urlOptions, postOptions), callback);
  };

  var downgradeUser = (postOptions, callback) => {
    if (!postOptions || !postOptions.body || postOptions.body.seatType === undefined) {
      throw new Error('seatType must be provided in the body for downgradeUser');
    }
    var urlOptions = { url: buildUserDowngradeUrl(postOptions) };
    return requestor.post(_.extend({}, optionsToSend, urlOptions, postOptions), callback);
  };
  ...
  var buildUserUpgradeUrl = (urlOptions) =>
