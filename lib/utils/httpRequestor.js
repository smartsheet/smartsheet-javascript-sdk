...
exports.create = function (requestorConfig) {
  ...
  var request =
    requestorConfig.request ||
    Promise.promisifyAll(require("axios"), { multiArgs: true });

  var handleResponse =
    requestorConfig.handleResponse || require("./responseHandler");
  ...
  var post = (options, callback) =>
    methodRequest(options, request.post, "POST", callback, options.body);
  ...
  var put = (options, callback) =>
    methodRequest(options, request.put, "PUT", callback, options.body);
  ...
  var makeRequestWithRetries = (
    ...
    effectiveRetryOptions.endRetryTime =
      Date.now() + effectiveRetryOptions.maxRetryDurationMillis;
  ...
  var retryWithBackoffHelper = (
    ...
    return (error) => {
      ...
      if (retryOptions.calcRetryBackoff) {
        backoffMillis = retryOptions.calcRetryBackoff(
          numRetries,
          processedError
        );
      } else {
