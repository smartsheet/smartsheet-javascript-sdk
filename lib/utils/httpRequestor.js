...
exports.create = function (requestorConfig) {
  ...
  var request = requestorConfig.request || Promise.promisifyAll(require('axios'), { multiArgs: true });

  var handleResponse = requestorConfig.handleResponse || require('./responseHandler');

  const proxy = requestorConfig.proxy;
  ...
  var get = (options, callback) => methodRequest(options, request.get, 'GET', callback);
  ...
  var deleteFunc = (options, callback) => methodRequest(options, request.delete, 'DELETE', callback);
