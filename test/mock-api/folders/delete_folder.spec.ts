...
describe('Folders - deleteFolder endpoint tests', () => {
    const client = createClient();

    it('deleteFolder generated url is correct', async () => {
        const requestId = crypto.randomUUID();
        const options = {
            folderId: TEST_FOLDER_ID,
            customProperties: {
                'x-request-id': requestId,
                'x-test-name': '/folders/delete-folder/all-response-body-properties'
            }
        };
    ...
    it('deleteFolder error 400 response', async () => {
        const requestId = crypto.randomUUID();
        const options = {
            folderId: TEST_FOLDER_ID,
            customProperties: {
