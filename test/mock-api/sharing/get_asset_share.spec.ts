...
describe('Sharing - getAssetShare endpoint tests', () => {
  const client = createClient();

  it('getAssetShare generated url is correct', async () => {
    ...
    const options = {
      ...
      customProperties: {
        ...
        'x-test-name': '/sharing/get-asset-share/all-response-body-properties',
      },
    };
    await client.sharing.getAssetShare(options);
  ...
  it('getAssetShare all response body properties', async () => {
    ...
    const response = await client.sharing.getAssetShare(options);
    const matchedRequest = await findWireMockRequest(requestId);
  ...
  it('getAssetShare required response body properties', async () => {
    const requestId = crypto.randomUUID();
    const options = {
      shareId: TEST_SHARE_ID,
      queryParameters: {
      ...
      customProperties: {
        'x-request-id': requestId,
        'x-test-name': '/sharing/get-asset-share/required-response-body-properties',
      },
    };
    const response = await client.sharing.getAssetShare(options);
    const matchedRequest = await findWireMockRequest(requestId);
  ...
  it('getAssetShare error 400 response', async () => {
    const requestId = crypto.randomUUID();
    const options = {
      shareId: TEST_SHARE_ID,
      queryParameters: {
  ...
  it('getAssetShare error 500 response', async () => {
    const requestId = crypto.randomUUID();
    const options = {
      shareId: TEST_SHARE_ID,
      queryParameters: {
